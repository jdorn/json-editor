...
if("function"==typeof b&&"function"==typeof c||b instanceof Date&&c instanceof Date||b instanceof RegExp&&c instanceof RegExp||b instanceof String&&c instanceof String||b instanceof Number&&c instanceof Number)return b.toString()===c.toString();
// At last checking prototypes as good as we can
if(!(b instanceof Object&&c instanceof Object))return!1;if(b.isPrototypeOf(c)||c.isPrototypeOf(b))return!1;if(b.constructor!==c.constructor)return!1;if(b.prototype!==c.prototype)return!1;
// Check for infinitive linking loops
if(d.indexOf(b)>-1||e.indexOf(c)>-1)return!1;
// Quick checking of one object being a subset of another.
// todo: cache the structure of arguments[0] for performance
for(f in c){if(c.hasOwnProperty(f)!==b.hasOwnProperty(f))return!1;if(typeof c[f]!=typeof b[f])return!1}for(f in b){if(c.hasOwnProperty(f)!==b.hasOwnProperty(f))return!1;if(typeof c[f]!=typeof b[f])return!1;switch(typeof b[f]){case"object":case"function":if(d.push(b),e.push(c),!a(b[f],c[f]))return!1;d.pop(),e.pop();break;default:if(b[f]!==c[f])return!1}}return!0}var b,c,d,e;if(arguments.length<1)return!0;for(b=1,c=arguments.length;b<c;b++)if(d=[],//Todo: this can be cached
e=[],!a(arguments[0],arguments[b]))return!1;return!0},g=function(a,b){if(!(a instanceof Element))throw new Error("element should be an instance of Element");b=c({},g.defaults.options,b||{}),this.element=a,this.options=b,this.init()};g.prototype={
...
var c=a.getEditorClass(a.schema);a.root=a.createEditor(c,{jsoneditor:a,schema:a.schema,required:!0,container:a.root_container}),a.root.preBuild(),a.root.build(),a.root.postBuild(),a.initValue=a.root.getValue(),
// Starting data
a.options.startval&&a.root.setValue(a.options.startval),a.validation_results=a.validator.validate(a.root.getValue()),"never"!==a.options.show_errors&&a.root.showValidationErrors(a.validation_results),a.ready=!0,
// Fire ready event asynchronously
window.requestAnimationFrame(function(){a.ready&&(a.validation_results=a.validator.validate(a.root.getValue()),"never"!==a.options.show_errors&&a.root.showValidationErrors(a.validation_results),a.trigger("ready"),a.trigger("change"))})})},getValue:function(){if(!this.ready)throw"JSON Editor not ready yet.  Listen for 'ready' event before getting the value";return this.root.getValue()},isEmpty:function(){if(!this.ready)throw"JSON Editor not ready yet.  Listen for 'ready' event before getting the value";return f(this.initValue,this.getValue())},setValue:function(a){if(!this.ready)throw"JSON Editor not ready yet.  Listen for 'ready' event before setting the value";return this.root.setValue(a),this},validate:function(a){if(!this.ready)throw"JSON Editor not ready yet.  Listen for 'ready' event before validating";
// Custom value
// Custom value
...
// Fire change event
a.trigger("change"))}),this}},compileTemplate:function(a,b){b=b||g.defaults.template;var c;
...
f[a]=c.concat(b[a]).reduce(function(a,b){return a.indexOf(b)<0&&a.push(b),a},[]):f[a]=c}),d(b,function(b,c){"undefined"==typeof a[b]&&(f[b]=c)}),f}},g.defaults={themes:{},templates:{},iconlibs:{},editors:{},languages:{},resolvers:[],custom_validators:[]},g.Validator=a.extend({init:function(a,b,c){this.jsoneditor=a,this.schema=b||this.jsoneditor.schema,this.options=c||{},this.translate=this.jsoneditor.translate||g.defaults.translate},validate:function(a){return this._validateSchema(this.schema,a)},_validateSchema:function(a,b,e){var f,h,i,j=this,k=[],l=JSON.stringify(b);/*
...
// `enum`
if(a["enum"]){for(f=!1,h=0;h<a["enum"].length;h++)l===JSON.stringify(a["enum"][h])&&(f=!0);f||k.push({path:e,property:"enum",message:this.translate("error_enum")})}
// `extends` (version 3)
...
// `anyOf`
if(a.anyOf){for(f=!1,h=0;h<a.anyOf.length;h++)if(!this._validateSchema(a.anyOf[h],b,e).length){f=!0;break}f||k.push({path:e,property:"anyOf",message:this.translate("error_anyOf")})}
// `oneOf`
if(a.oneOf){f=0;var m=[];for(h=0;h<a.oneOf.length;h++){
// Set the error paths to be path.oneOf[i].rest.of.path
var n=this._validateSchema(a.oneOf[h],b,e);for(n.length||f++,i=0;i<n.length;i++)n[i].path=e+".oneOf["+h+"]"+n[i].path.substr(e.length);m=m.concat(n)}1!==f&&(k.push({path:e,property:"oneOf",message:this.translate("error_oneOf",[f])}),k=k.concat(m))}
// `type` (both Version 3 and Version 4 support)
if(
// `not`
a.not&&(this._validateSchema(a.not,b,e).length||k.push({path:e,property:"not",message:this.translate("error_not")})),a.type)
// Union type
if(Array.isArray(a.type)){for(f=!1,h=0;h<a.type.length;h++)if(this._checkType(a.type[h],b)){f=!0;break}f||k.push({path:e,property:"type",message:this.translate("error_type_union")})}else this._checkType(a.type,b)||k.push({path:e,property:"type",message:this.translate("error_type",[a.type])});
// `disallow` (version 3)
if(a.disallow)
// Union type
if(Array.isArray(a.disallow)){for(f=!0,h=0;h<a.disallow.length;h++)if(this._checkType(a.disallow[h],b)){f=!1;break}f||k.push({path:e,property:"disallow",message:this.translate("error_disallow_union")})}else this._checkType(a.disallow,b)&&k.push({path:e,property:"disallow",message:this.translate("error_disallow",[a.disallow])});/*
     * Type Specific Validation
     */
var o,p;
// Number Specific Validation
if("number"!=typeof b&&"string"!=typeof b||(o=this.jsoneditor.options.stringWhiteLabel,p=b.startsWith&&b.startsWith(o)),"number"==typeof b){
// `multipleOf` and `divisibleBy`
if(a.multipleOf||a.divisibleBy){var q=a.multipleOf||a.divisibleBy;
// Vanilla JS, prone to floating point rounding errors (e.g. 1.14 / .01 == 113.99999)
f=b/q===Math.floor(b/q),
// Use math.js is available
window.math?f=window.math.mod(window.math.bignumber(b),window.math.bignumber(q)).equals(0):window.Decimal&&(f=new window.Decimal(b).mod(new window.Decimal(q)).equals(0)),f||k.push({path:e,property:a.multipleOf?"multipleOf":"divisibleBy",message:this.translate("error_multipleOf",[q])})}
// `maximum`
a.hasOwnProperty("maximum")&&(
// Vanilla JS, prone to floating point rounding errors (e.g. .999999999999999 == 1)
f=a.exclusiveMaximum?b<a.maximum:b<=a.maximum,
// Use math.js is available
window.math?f=window.math[a.exclusiveMaximum?"smaller":"smallerEq"](window.math.bignumber(b),window.math.bignumber(a.maximum)):window.Decimal&&(f=new window.Decimal(b)[a.exclusiveMaximum?"lt":"lte"](new window.Decimal(a.maximum))),f||k.push({path:e,property:"maximum",message:this.translate(a.exclusiveMaximum?"error_maximum_excl":"error_maximum_incl",[a.maximum])})),
// `minimum`
a.hasOwnProperty("minimum")&&(
// Vanilla JS, prone to floating point rounding errors (e.g. .999999999999999 == 1)
f=a.exclusiveMinimum?b>a.minimum:b>=a.minimum,
// Use math.js is available
window.math?f=window.math[a.exclusiveMinimum?"larger":"largerEq"](window.math.bignumber(b),window.math.bignumber(a.minimum)):window.Decimal&&(f=new window.Decimal(b)[a.exclusiveMinimum?"gt":"gte"](new window.Decimal(a.minimum))),f||k.push({path:e,property:"minimum",message:this.translate(a.exclusiveMinimum?"error_minimum_excl":"error_minimum_incl",[a.minimum])}))}else"string"==typeof b&&(
// `maxLength`
a.maxLength&&!p&&(b+"").length>a.maxLength&&k.push({path:e,property:"maxLength",message:this.translate("error_maxLength",[a.maxLength])}),
// `minLength`
a.minLength&&!p&&(b+"").length<a.minLength&&k.push({path:e,property:"minLength",message:this.translate(1===a.minLength?"error_notempty":"error_minLength",[a.minLength])}),
// `pattern`
a.pattern&&!p&&(new RegExp(a.pattern).test(b)||k.push({path:e,property:"pattern",message:this.translate("error_pattern",[a.pattern])})));if("url"!==a.format||p){if("object"==typeof b&&null!==b&&Array.isArray(b)){
...
// `minItems`
a.minItems&&b.length<a.minItems&&k.push({path:e,property:"minItems",message:this.translate("error_minItems",[a.minItems])}),a.uniqueItems){var r={};for(h=0;h<b.length;h++){if(f=JSON.stringify(b[h]),r[f]){k.push({path:e,property:"uniqueItems",message:this.translate("error_uniqueItems")});break}r[f]=!0}}}else if("object"==typeof b&&null!==b){
// `maxProperties`
if(a.maxProperties){f=0;for(h in b)b.hasOwnProperty(h)&&f++;f>a.maxProperties&&k.push({path:e,property:"maxProperties",message:this.translate("error_maxProperties",[a.maxProperties])})}
// `minProperties`
if(a.minProperties){f=0;for(h in b)b.hasOwnProperty(h)&&f++;f<a.minProperties&&k.push({path:e,property:"minProperties",message:this.translate("error_minProperties",[a.minProperties])})}
// Version 4 `required`
if(a.required&&Array.isArray(a.required))for(h=0;h<a.required.length;h++)"undefined"==typeof b[a.required[h]]&&k.push({path:e,property:"required",message:this.translate("error_required",[a.properties[a.required[h]].title||a.required[h]])});
// `properties`
var s={};if(a.properties)for(h in a.properties)a.properties.hasOwnProperty(h)&&(s[h]=!0,k=k.concat(this._validateSchema(a.properties[h],b[h],e+"."+h)));
// `patternProperties`
if(a.patternProperties)for(h in a.patternProperties)if(a.patternProperties.hasOwnProperty(h)){var t=new RegExp(h);
// Check which properties match
for(i in b)b.hasOwnProperty(i)&&t.test(i)&&(s[i]=!0,k=k.concat(this._validateSchema(a.patternProperties[h],b[i],e+"."+i)))}
// `additionalProperties`
if(
// The no_additional_properties option currently doesn't work with extended schemas that use oneOf or anyOf
"undefined"!=typeof a.additionalProperties||!this.jsoneditor.options.no_additional_properties||a.oneOf||a.anyOf||(a.additionalProperties=!1),"undefined"!=typeof a.additionalProperties)for(h in b)if(b.hasOwnProperty(h)&&!s[h]){
...
// Property dependency
if(Array.isArray(a.dependencies[h]))for(i=0;i<a.dependencies[h].length;i++)"undefined"==typeof b[a.dependencies[h][i]]&&k.push({path:e,property:"dependencies",message:this.translate("error_dependency",[a.dependencies[h][i]])});else k=k.concat(this._validateSchema(a.dependencies[h],b,e))}}else{var u=/(http|https):\/\/(\w+:{0,1}\w*)?(\S+)(:[0-9]+)?(\/|\/([\w#!:.?+=&%!\-\/]))?/;u.test(b)||k.push({path:e,property:"pattern",message:this.translate("error_url",[a.title||"Value"])})}
// Custom type validation (global)
// Custom type validation (instance specific)
return d(g.defaults.custom_validators,function(c,d){k=k.concat(d.call(j,a,b,e))}),this.options.custom_validators&&d(this.options.custom_validators,function(c,d){k=k.concat(d.call(j,a,b,e))}),k},_checkType:function(a,b){
// Simple types
// Simple types
...
b.input.value=a.html(),b.value=b.input.value,b.is_dirty=!0,b.onChange(!0)});else if("markdown"===this.input_type&&window.EpicEditor)this.epiceditor_container=document.createElement("div"),this.input.parentNode.insertBefore(this.epiceditor_container,this.input),this.input.style.display="none",a=c({},g.plugins.epiceditor,{container:this.epiceditor_container,clientSideStorage:!1}),this.epiceditor=new window.EpicEditor(a).load(),this.epiceditor.importFile(null,this.getValue()),this.epiceditor.on("update",function(){var a=b.epiceditor.exportFile();b.input.value=a,b.value=a,b.is_dirty=!0,b.onChange(!0)});else if(window.ace){var d=this.input_type;
// aliases for c/cpp
...
// Show/Hide button
this.collapsed=!1,this.toggle_button=this.getButton("","collapse",this.translate("button_collapse")),this.title_controls.appendChild(this.toggle_button);var c=function(b){b.preventDefault(),b.stopPropagation(),a.collapsed?(a.editor_holder.style.display="",a.collapsed=!1,a.setButtonText(a.toggle_button,"","collapse",a.translate("button_collapse"))):(a.editor_holder.style.display="none",a.collapsed=!0,a.setButtonText(a.toggle_button,"","expand",a.translate("button_expand")))};this.toggle_button.addEventListener("click",c),(this.options.collapse_on_title_click||this.jsoneditor.options.collapse_on_title_click)&&this.title.querySelector(".headerText").addEventListener("click",c),
...
b||(c.refreshValue(),c.layoutEditors())}},onChildEditorChange:function(a){this.refreshValue(),this._super(a)},canHaveAdditionalProperties:function(){return"boolean"==typeof this.schema.additionalProperties?this.schema.additionalProperties:!this.jsoneditor.options.no_additional_properties},destroy:function(){d(this.cached_editors,function(a,b){b.destroy()}),this.editor_holder&&(this.editor_holder.innerHTML=""),this.title&&this.title.parentNode&&this.title.parentNode.removeChild(this.title),this.error_holder&&this.error_holder.parentNode&&this.error_holder.parentNode.removeChild(this.error_holder),this.editors=null,this.cached_editors=null,this.editor_holder&&this.editor_holder.parentNode&&this.editor_holder.parentNode.removeChild(this.editor_holder),this.editor_holder=null,this._super()},getValue:function(){var a=this._super();if(this.jsoneditor.options.remove_empty_properties||this.options.remove_empty_properties)for(var b in a)a.hasOwnProperty(b)&&("boolean"==typeof a[b]||a[b]||delete a[b]);if(this.jsoneditor.options.remove_empty_array||this.options.remove_empty_array)for(var c in a)a.hasOwnProperty(c)&&a[c]&&a[c]instanceof Array&&0===a[c].length&&delete a[c];return a},refreshValue:function(){this.value={};for(var a in this.editors)this.editors.hasOwnProperty(a)&&(this.value[a]=this.editors[a].getValue());this.adding_property&&this.refreshAddProperties()},refreshAddProperties:function(){if(this.options.disable_properties||this.options.disable_properties!==!1&&this.jsoneditor.options.disable_properties)return void(this.addproperty_controls.style.display="none");var a,b=!1,c=!1,d=0,e=!1;
...
c.hide_delete_buttons||(c.rows[e].delete_button=this.getButton("","delete",this.translate("button_delete_row_title",[c.getItemTitle()])),c.rows[e].delete_button.className+=" delete delete-array-element",c.rows[e].delete_button.setAttribute("data-i",e),c.rows[e].delete_button.addEventListener("click",function(a){a.preventDefault(),a.stopPropagation();var b=1*this.getAttribute("data-i"),e=c.getValue(),f=[],g=null;d(e,function(a,d){
// If the one we're deleting is the active tab
// Make the next tab active if there is one
// Note: the next tab is going to be the current tab after deletion
return a===b?void(c.rows[a].tab===c.active_tab&&(c.rows[a+1]?g=c.rows[a].tab:a&&(g=c.rows[a-1].tab))):void f.push(d)}),c.setValue(f),g&&(c.active_tab=g,c.refreshTabs()),c.onChange(!0)}),c.rows[e].editor_holder?c.rows[e].editor_holder.appendChild(c.rows[e].delete_button):c.rows[e].title?c.rows[e].title.appendChild(c.rows[e].delete_button):c.rows[e].control&&(c.rows[e].control.appendChild(c.rows[e].delete_button),c.rows[e].delete_button.className+=" inline-delete",c.rows[e].control.className+=" inline-input")),e&&!c.hide_move_buttons&&(c.rows[e].moveup_button=this.getButton("","moveup",this.translate("button_move_up_title")),c.rows[e].moveup_button.className+=" moveup",c.rows[e].moveup_button.setAttribute("data-i",e),c.rows[e].moveup_button.addEventListener("click",function(a){a.preventDefault(),a.stopPropagation();var b=1*this.getAttribute("data-i");if(!(b<=0)){var d=c.getValue(),e=d[b-1];d[b-1]=d[b],d[b]=e,c.setValue(d),c.active_tab=c.rows[b-1].tab,c.refreshTabs(),c.onChange(!0)}}),f&&f.appendChild(c.rows[e].moveup_button)),c.hide_move_buttons||(c.rows[e].movedown_button=this.getButton("","movedown",this.translate("button_move_down_title")),c.rows[e].movedown_button.className+=" movedown",c.rows[e].movedown_button.setAttribute("data-i",e),c.rows[e].movedown_button.addEventListener("click",function(a){a.preventDefault(),a.stopPropagation();var b=1*this.getAttribute("data-i"),d=c.getValue();if(!(b>=d.length-1)){var e=d[b+1];d[b+1]=d[b],d[b]=e,c.setValue(d),c.active_tab=c.rows[b+1].tab,c.refreshTabs(),c.onChange(!0)}}),f&&f.appendChild(c.rows[e].movedown_button)),a&&c.rows[e].setValue(a,b),c.refreshTabs()},addControls:function(){
//var controls_display = self.controls.style.display;
function a(a){a.preventDefault(),a.stopPropagation(),b.collapsed?(b.collapsed=!1,b.panel&&(b.panel.style.display=""),b.row_holder.style.display=c,b.tabs_holder&&(b.tabs_holder.style.display=""),b.controls.style.display="",b.setButtonText(b.toggle_button,"","collapse",b.translate("button_collapse"))):(b.collapsed=!0,b.row_holder.style.display="none",b.tabs_holder&&(b.tabs_holder.style.display="none"),b.controls.style.display="none",b.panel&&(b.panel.style.display="none"),b.setButtonText(b.toggle_button,"","expand",b.translate("button_expand")))}var b=this;this.collapsed=!1,this.toggle_button=this.getButton("","collapse",this.translate("button_collapse")),this.title_controls.appendChild(this.toggle_button);var c=b.row_holder.style.display;this.toggle_button.addEventListener("click",a),(this.options.collapse_on_title_click||this.jsoneditor.options.collapse_on_title_click)&&this.title.querySelector(".headerText").addEventListener("click",a),
// If it should start collapsed
this.options.collapsed&&e(this.toggle_button,"click"),
// Collapse button disabled
this.schema.options&&"undefined"!=typeof this.schema.options.disable_collapse?this.schema.options.disable_collapse&&(this.toggle_button.style.display="none"):this.jsoneditor.options.disable_collapse&&(this.toggle_button.style.display="none");
// Add "new row" and "delete last" buttons below editor
var d=this.translate("button_add_row_title",[this.getItemTitle()]);this.add_row_button=this.getButton(d,"add",d),this.add_row_button.className+=" add-array-element",this.add_row_button.addEventListener("click",function(a){a.preventDefault(),a.stopPropagation();var c=b.rows.length;b.row_cache[c]?(b.rows[c]=b.row_cache[c],b.rows[c].setValue(b.rows[c].getDefault(),!0),b.rows[c].container.style.display="",b.rows[c].tab&&(b.rows[c].tab.style.display=""),b.rows[c].register()):b.addRow(),b.active_tab=b.rows[c].tab,b.refreshTabs(),b.refreshValue(),b.onChange(!0)}),b.controls.appendChild(this.add_row_button),this.delete_last_row_button=this.getButton(this.translate("button_delete_last",[this.getItemTitle()]),"delete",this.translate("button_delete_last_title",[this.getItemTitle()])),this.delete_last_row_button.addEventListener("click",function(a){a.preventDefault(),a.stopPropagation();var c=b.getValue(),d=null;b.rows.length>1&&b.rows[b.rows.length-1].tab===b.active_tab&&(d=b.rows[b.rows.length-2].tab),c.pop(),b.setValue(c),d&&(b.active_tab=d,b.refreshTabs()),b.onChange(!0)}),b.controls.appendChild(this.delete_last_row_button),this.remove_all_rows_button=this.getButton(this.translate("button_delete_all"),"delete",this.translate("button_delete_all_title")),this.remove_all_rows_button.addEventListener("click",function(a){a.preventDefault(),a.stopPropagation(),b.setValue([]),b.onChange(!0)}),b.controls.appendChild(this.remove_all_rows_button),b.tabs&&(this.add_row_button.style.width="100%",this.add_row_button.style.textAlign="left",this.add_row_button.style.marginBottom="3px",this.delete_last_row_button.style.width="100%",this.delete_last_row_button.style.textAlign="left",this.delete_last_row_button.style.marginBottom="3px",this.remove_all_rows_button.style.width="100%",this.remove_all_rows_button.style.textAlign="left",this.remove_all_rows_button.style.marginBottom="3px"),b.controls.className+=" button-box-row"},showValidationErrors:function(a){var b=this,c=[],e=[];
// Show errors for this editor
if(d(a,function(a,d){d.path===b.path?c.push(d):e.push(d)}),this.error_holder)if(c.length){this.error_holder.innerHTML="",this.error_holder.style.display="",d(c,function(a,c){b.error_holder.appendChild(b.theme.getErrorMessage(c.message))})}else this.error_holder.style.display="none";
// Show errors for child editors
...
this.switcher=this.theme.getSwitcher(this.display_text),this.switcher.className+=" switcher ",this.editor_holder=document.createElement("div"),this.editor_holder.className+="switch-holder",this.editor_holder.style.position="relative",this.editor_holder.appendChild(this.switcher),b.appendChild(this.editor_holder),this.switcher.addEventListener("change",function(b){b.preventDefault(),b.stopPropagation(),a.switchEditor(a.display_text.indexOf(this.value)),a.onChange(!0)});var e={};a.jsoneditor.options.custom_validators&&(e.custom_validators=a.jsoneditor.options.custom_validators),this.switcher_options=this.theme.getSwitcherOptions(this.switcher),d(this.types,function(b,d){a.editors[b]=!1;var f;"string"==typeof d?(f=c({},a.schema),f.type=d):(f=c({},a.schema,d),
// If we need to merge `required` arrays
d.required&&Array.isArray(d.required)&&a.schema.required&&Array.isArray(a.schema.required)&&(f.required=a.schema.required.concat(d.required))),a.validators[b]=new g.Validator(a.jsoneditor,f,e)}),this.switchEditor(0)},onChildEditorChange:function(a){this.editors[this.type]&&(this.refreshValue(),this.refreshHeaderText()),this._super()},refreshHeaderText:function(){var a=this.getDisplayText(this.types);d(this.switcher_options,function(b,c){c.textContent=a[b]})},refreshValue:function(){this.value=this.editors[this.type].getValue()},setValue:function(a,b){
...
error_minLength:"Value must be at least {{0}} characters long",/**
   ...
   */
error_pattern:"Value must match the pattern {{0}}",/*URL */
...
g.plugins={ace:{theme:""},epiceditor:{},sceditor:{},select2:{},selectize:{}},
// Default per-editor options
d(g.defaults.editors,function(a,b){g.defaults.editors[a].options=b.options||{}}),
// Set the default resolvers
// Use "multiple" as a fall back for everything
g.defaults.resolvers.unshift(function(a){if("string"!=typeof a.type)return"multiple"}),
// If the type is not set but properties are defined, we can infer the type is actually object
g.defaults.resolvers.unshift(function(a){
...
// Boolean editors
g.defaults.resolvers.unshift(function(a){if("boolean"===a.type)
// If explicitly set to 'checkbox', use that
// If explicitly set to 'checkbox', use that
return"checkbox"===a.format||a.options&&a.options.checkbox?"checkbox":g.plugins.selectize.enable?"selectize":"select"}),
...
g.defaults.resolvers.unshift(function(a){if("string"===a.type&&"url"===a.format&&a.options&&a.options.upload===!0&&window.FileReader)return"upload"}),
// Use the table editor for arrays with the format set to `table`
g.defaults.resolvers.unshift(function(a){
// Type `array` with format set to `table`
if("array"==a.type&&"table"==a.format)return"table"}),
// Use the `select` editor for dynamic enumSource enums
g.defaults.resolvers.unshift(function(a){if(a.enumSource)return g.plugins.selectize.enable?"selectize":"select"}),
// Use the `enum` or `select` editors for schemas with enumerated properties
g.defaults.resolvers.unshift(function(a){if(a["enum"]){if("array"===a.type||"object"===a.type)return"enum";if("number"===a.type||"integer"===a.type||"string"===a.type)return g.plugins.selectize.enable?"selectize":"select"}}),
// Specialized editors for arrays of strings
g.defaults.resolvers.unshift(function(a){if("array"===a.type&&a.items&&!Array.isArray(a.items)&&a.uniqueItems&&["string","number","integer"].indexOf(a.items.type)>=0){
// For enumerated strings, number, or integers
if(a.items["enum"])return"multiselect";if(g.plugins.selectize.enable&&"string"===a.items.type)return"arraySelectize"}}),
 ...
 */
function(){if(window.jQuery||window.Zepto){var a=window.jQuery||window.Zepto;a.jsoneditor=g.defaults,a.fn.jsoneditor=function(a){var b=this,c=this.data("jsoneditor");if("value"===a){if(!c)throw"Must initialize jsoneditor before getting/setting the value";
// Set value
...
c.on("change",function(){b.trigger("change")}),c.on("ready",function(){b.trigger("ready")}))}return this}}}(),window.JSONEditor=g}();
